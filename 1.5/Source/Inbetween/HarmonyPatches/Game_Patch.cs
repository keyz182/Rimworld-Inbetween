...
public static class Game_Patch
{
    ...
    public static bool InitNewGame_Patch(Game __instance)
    {
      ...
      Log.Message("Initializing new game with mods:\n" + LoadedModManager.RunningMods.Select<ModContentPack, string>((Func<ModContentPack, string>) (mod => mod.PackageIdPlayerFacing + (!mod.ModMetaData.VersionCompatible ? " (incompatible version)" : ""))).ToLineList("  - "));
      ...
      else
      {
        ...
        try
        {
          ...
          Map map = MapGenerator.GenerateMap(mapSize, (MapParent) parent, InbetweenDefOf.IB_Empty);//, parent.ExtraGenStepDefs);
